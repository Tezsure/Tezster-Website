...
async function GetMainnetNodes() {
  try {
    ...
    const config = {
      method: "get",
      url: "https://api.tzkt.io/v1/statistics/current",
      headers: {},
    };
    const response = await axios(config);
    const LastBlock = response.data.level;
    const BlockTime = response.data.timestamp;
    debugger;
    document.getElementById(
      "block_level_mainnet"
    ).innerText = LastBlock.toString();
    document.getElementById("last_block_mainnet").innerText = new Date(
      BlockTime
    ).toString();
    setTimeout(() => {
...
async function GetDelphinetNodes() {
  try {
    ...
    const config = {
      method: "get",
      url: "https://api.delphinet.tzkt.io/v1/statistics/current",
      headers: {},
    };
    const response = await axios(config);
    const LastBlock = response.data.level;
    const BlockTime = response.data.timestamp;
    document.getElementById(
      "block_level_delphinet"
    ).innerText = LastBlock.toString();
    document.getElementById("last_block_delphinet").innerText = new Date(
      BlockTime
    ).toString();
    setTimeout(() => {
      spinner.classList.remove("rotating");
